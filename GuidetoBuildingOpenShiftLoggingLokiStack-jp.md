承知いたしました。実際に適用した各YAMLファイルの内容を正確に反映し、一から構築・復旧するための**決定版・構築手順書**としてまとめ直しました。

この手順は、特に **Loki 6.x系 (LokiStack)** と **Vector (ClusterLogForwarder)** の組み合わせで発生しやすい「権限問題」「URL構造の不一致」「リソース不足」をすべて回避するように構成されています。
